            ...
            <script>
                ...
                $(document).ready(function () {
                    ...
                    function onAddSectionClick(e) {
                        ...
                        var sectionDiameterSpinnerEdit = $("#sectionDiameter_Spinner").data("kendoNumericTextBox");
                        //var sectionDiameterSpinnerEditValue = sectionDiameterSpinnerEdit.value();
                        sectionDiameterSpinnerEdit.value(0);

                        var sectionDiameterSliderEdit = $("#sectionDiameter_Slider").data("kendoSlider");
                        //var sectionDiameterSliderEditValue = sectionDiameterSliderEdit.value();
                        sectionDiameterSliderEdit.value(0);

                        var sectionLengthSpinnerEdit = $("#sectionLength_Spinner").data("kendoNumericTextBox");
                        sectionLengthSpinnerEdit.value(0);

                        var sectionLengthSliderEdit = $("#sectionLength_Slider").data("kendoSlider");
                        sectionLengthSliderEdit.value(0);

                        var innerPortDiameterSpinnerEdit = $("#innerPortDiameter_Spinner").data("kendoNumericTextBox");
                        innerPortDiameterSpinnerEdit.value(0);

                        var innerPortDiameterSliderEdit = $("#innerPortDiameter_Slider").data("kendoSlider");
                        innerPortDiameterSliderEdit.value(0);
                    }
                    ...
                    function onRemoveSectionClick(e) {
                        if (selectedSection != null) {
                        } else {
                            alert("No section is selected");
                        }
                    ...
                    function onOKEditSectionWindowClick(e) {
                        ...
                        if (sectionDiameterValue != 0 && sectionLengthValue != 0 && innerPortDiameterValue != 0 && innerPortDiameterValue < sectionDiameterValue) {
                            ...
                            for (var i = 0; i < motorSections.length; i++) {
                                var sectionId = motorSections[i].sectionId;
                                var sectionG = SVGdraw.select('rect[id=section_' + sectionId + ']');
                                var innerPortG = SVGdraw.select('rect[id=section_inner_' + sectionId + ']');



                                if (sectionId == selectedSection.sectionId) {
                                    motor.setSection(selectedSection, selectedSection.sectionId);
                                    if (rocketDiameterValue >= sectionDiameterValue) {
                                        sectionG.attr({"width": sectionLengthValue, "height": sectionDiameterValue, "y": rockety + 0.5 * Math.abs(rocketDiameterValue - sectionDiameterValue)});
                                    } else {
                                        sectionG.attr({"width": sectionLengthValue, "height": sectionDiameterValue, "y": rockety - 0.5 * Math.abs(rocketDiameterValue - sectionDiameterValue)});
                                    }
                                    if (rocketDiameterValue >= innerPortDiameterValue) {
                                        innerPortG.attr({"width": sectionLengthValue, "height": innerPortDiameterValue, "y": rockety + 0.5 * Math.abs(rocketDiameterValue - innerPortDiameterValue)});
                                    } else {
                                        innerPortG.attr({"width": sectionLengthValue, "height": innerPortDiameterValue, "y": rockety - 0.5 * Math.abs(rocketDiameterValue - innerPortDiameterValue)});
                                    }
                                }
                                sectionG.attr({"x": rocketX + totalLen});
                                innerPortG.attr({"x": rocketX + totalLen});
